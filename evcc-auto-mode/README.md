...
- Prozent-Sensoren werden jetzt fuer SoC-Felder gezielt gesucht
- Fehler aus der Prozent-Sensorliste werden in der UI sichtbar gemacht

Neu in `0.3.11`:

- beim Deaktivieren von `Max PV` wird `minCurrent` jetzt wieder aktiv auf den konfigurierten Mindestwert zurueckgesetzt

Neu in `0.3.10`:

- optionaler Home-Assistant-Sensor fuer Hausbatterie-SoC hinzugefuegt
- wenn gesetzt, wird dieser fuer `batterySoC` priorisiert
- bei leerem Feld bleibt `evcc/site/batterySoc` aktiv, damit MQTT weiter als Fallback funktioniert
...
- beim Deaktivieren wird auf den zuvor gemerkten Modus zurueckgeschaltet
- die `minCurrent`-Regelung laeuft jetzt auch ohne `auto_mode_active`, sobald `Max PV` aktiv ist und evcc in `minpv` steht

Neu in `0.3.7`:

- `Controls` weiter verdichtet: kompakte Status-Tiles statt mehrerer Textbloecke
- nur die wichtigsten Zusatzinfos bleiben sichtbar, z. B. letzter What-If-Befehl und berechneter Max-PV-Strom
